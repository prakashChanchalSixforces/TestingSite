...
import {getElectricianServiceDetails } from '../../../../store/Actions/Dashboard.actions';

function ElectriciansPricing(){
const [activetoggle,setActivetoggle]=useState('Yes')
const [saved,setSaved]=useState(false)
let electriciandata = useSelector(state => state?.dashboardReducer?.electriciansService);

const [values,setValues]=useState(
    {
"hourlyRate":electriciandata?.hourlyRate,
"minimumOrder":electriciandata?.minimumOrder,
"serviceCall":electriciandata?.serviceCall,
"travelChargesPerHour":electriciandata?.travelChargesPerHour
    }
)
...
let dispatch = useDispatch()
   ...
   const handlesubmit=async()=>{
    if(isValid()){
    let res = await dispatch(getElectricianServiceDetails(values))
   if(res?.status===true)
   {
    setSaved(true)
   }
...
export default ElectriciansPricing
