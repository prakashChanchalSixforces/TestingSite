...
import {roofcleaning } from '../../../store/Actions/User.action';
...
else if(!(values.roofMaterial&&values.roofMaterial.length>0)){
    toast.error('Please select roof material')
    return false
}
...
const handleSubmit=async()=>{
    let res
    if(isValid()){
     res = await dispatch(roofcleaning(values,1,10));
     if(res.status===false){
...
<img src={Hrimg} alt='hr' className='mb-3 mt-3'/>
    ...
    <Dropdwn controlId="floatingSelect" label={<Label>How many floors is your house</Label>} >
      ...
      <Form.Group className="mb-3">
    <Dropdwn controlId="floatingSelect" label={<Label >Approx. sq. ft. of the house</Label>} >
      <Sel aria-label="Floating label select example"
      ...
      <Form.Group className="mb-3">
    <Dropdwn controlId="floatingSelect" label={<Label>Roof Material</Label>} >
      <Sel aria-label="Floating label select example"
      onChange={handlechange('roofMaterial')}
      >
      <option value="">Roof Material</option>
      <option value='Metal'>Metal</option>
        <option value='Shingles'>Shingles</option>
        <option value='Clay or concrete tile'>Clay or concrete tile</option>
      </Sel>
    </Dropdwn>
      </Form.Group>
      <hr/>
