...
import { hvac } from '../../../store/Actions/User.action';
...
const handleSubmit=async()=>{
    let res
    if(isValid()){
     res = await dispatch(hvac(values,1,10));
     if(res.status===false){
      ...
      <Details className='d-flex justify-content-between p-2'>
            <Info>
              <Smallfont>List of services that get resolved in 1-2 hours</Smallfont>
                <Text2>Top 10 HVAC problems (summary)</Text2>
                </Info>
                <div>
                <img src={infoLogo} alt='info' onClick={()=>{setView('hvac')}}/>
                </div>
      </Details>
      <Details className='d-flex justify-content-between p-2'>
            <Info>
              <Smallfont>List of services that get resolved in 1-2 hours</Smallfont>
                <Text2>Top 10 air conditioner problems</Text2>
                </Info>
                <div>
                <img src={infoLogo} alt='info' onClick={()=>{setView('air')}}/>
                </div>
      </Details>
      <hr/>
      <Search>
