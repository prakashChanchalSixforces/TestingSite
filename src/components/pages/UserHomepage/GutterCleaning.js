...
import { guttercleaning } from '../../../store/Actions/User.action';
...
let value= name === 'floorsInHouse' ? parseInt(event.target.value):name==='date'?moment(event).format('LL'):event.target.value
if(name==='date'){
...
else if(!(values.floorsInHouse||values.floorsInHouse===isNaN)){
    toast.error('Please select number of floors in your house')
    return false
}
else
return true
...
const handleSubmit=async()=>{
    let res
    if(isValid()){
     res = await dispatch(guttercleaning(values,1,10));
     if(res.status===false){
    ...
    <Details className='d-flex justify-content-start'>
            ...
            style={{
              paddingLeft:'12px',paddingTop:'10px',color:'black',fontSize:'14px',fontWeight:'500',fontFamily:'Roobert-medium'
                ...
                Service
                <Text>Gutter Cleaning and Roof Blowing</Text>
                </div>
        </Details>
        <br/>
...
<img src={Hrimg} alt='hr' className='mb-3 mt-3'/>
    ...
    <Dropdwn controlId="floatingSelect" label={<Label>How many floors is your house</Label>} >
      <Sel aria-label="Floating label select example"
      onChange={handlechange('floorsInHouse')}
      >
      ...
      <Form.Group className="mb-3">
    <Dropdwn controlId="floatingSelect" label={<Label >Approx. sq. ft. of the covered</Label>} >
      <Sel aria-label="Floating label select example"
      onChange={handlechange('approxSizeInSqFt')}
      >
      <option value="">Approx. sq. ft. of the covered </option>
        <option value={`< 1000 Sq. Ft.`}>{`< 1000 Sq. Ft.`}</option>
        <option value={`1000 Sq.Ft. to 1500 Sq.Ft.`}>{`1000 Sq.Ft. to 1500 Sq.Ft.`} </option>
        <option value={`1500 Sq.Ft. to 2000 Sq.Ft.`}>{`1500 Sq.Ft. to 2000 Sq.Ft.`}</option>
        <option value={`2000 Sq.Ft. to 2500 Sq.Ft.`}>{`2000 Sq.Ft. to 2500 Sq.Ft.`} </option>
        <option value={`2500 Sq.Ft. to 3000 Sq.Ft.`}>{`2500 Sq.Ft. to 3000 Sq.Ft.`}</option>
        <option value={`3000 Sq.Ft. to 3500 Sq.Ft.`}>{`3000 Sq.Ft. to 3500 Sq.Ft.`}</option>
        <option value={`> 3500 Sq.Ft +`}>{`> 3500 Sq.Ft +`}</option>
      </Sel>
    </Dropdwn>
      </Form.Group>
      <hr/>
