...
import {rugcleaning } from '../../../store/Actions/User.action';
...
if(!(values.address&&values.address.length>0)){
    ...
    toast.error('Please select number of rugs')
    return false
}
...
const handleSubmit = async()=>{
    let res
    if(isValid()){
     res = await dispatch(rugcleaning(values,1,10));
     if(res.status===false){
...
:''}
    ...
    <Dropdwn controlId="floatingSelect" label={<Label>Number of rugs to be cleaned</Label>} >
     ...
     onChange={handlechange('numberOfRooms')}
      >
      <option value="">Enter number of rugs</option>
            ...
            <Info>
              <Smallfont>Are there any smokers in the house?</Smallfont>
                <div className='d-flex justify-content-start mb-2'>
                    ...
                    <>
                 <Yesno size='md' variant={smoke==={x}?'dark':'outline-dark'} onClick={()=>{handlesmoke(x)}} key={index}>{x}</Yesno> &nbsp;&nbsp;
                 </>
                    )})}
                </div>
                </Info>
                <div>
                <img src={infoLogo} alt='hr' />
                </div>
      </Details>
      <Details className='d-flex justify-content-between p-2'>
            <Info>
              <Smallfont>Are there any pets in the house?</Smallfont>
                <div className='d-flex justify-content-start mb-2'>
                {pet.map((x,index)=>{return(
                    <>
                 <Yesno size='md' variant={pets==={x}?'dark':'outline-dark'} onClick={()=>{handlepets(x)}} key={index}>{x}</Yesno> &nbsp;&nbsp;
                 </>
                    )})}
                </div>
                </Info>
